...
import type { HomeAssistantFixed } from "./types";
...
export class LgRemoteControlEditor extends LitElement {
  ...
  setRepeatConfig(repeat) {
    ...
    const hasVolumeOverride =
      repeat?.volume_delay !== undefined || repeat?.volume_interval !== undefined;
    return html`
  ...
  getDeviceAVReceiverDropdown(optionvalue) {
    ...
    const blankEntity =
      !this._config.av_receiver_family || this._config.av_receiver_family === ""
  ...
  getMediaPlayerEntityDropdown(optionValue) {
    if (this._config.av_receiver_family) {
      ...
      const blankEntity =
        this._config.ampli_entity === "" || !mediaPlayerEntities.includes(optionValue)
          ? html`<option value="" selected>- - - -</option>`
          : "";
      return html`
